...
def find_rotate_rotatable_bond(lig3D, catoms):
    bats = list(set(lig3D.getBondedAtomsnotH(catoms[0])) | set(
        lig3D.getBondedAtomsnotH(catoms[1])))
    rb1 = 1000
    rb2 = 1000
    ...
    if (rb1 != 1000) and (rb2 != 1000):  # rotatable bond present, execute rotations
        ...
        while theta < 360:  # minimize distance between connecting atoms
            ...
            obj = distance(lig3D.getAtomCoords(
                catoms[1]), rotfrag3D.getAtomCoords(ncoord))
            obj = obj + \
                distance(lig3D.getAtomCoords(
                    catoms[0]), rotfrag3D.getAtomCoords(ncoord))
            if obj < objopt:
                thetaopt = theta
                objopt = obj
            theta = theta + dtheta
        ...
        for j in range(0, occs[i]):
            ...
            if not(ligand == 'x' or ligand == 'X') and (totlig-1+denticity < coord):
                ...
                if args.tsgen and (ligand.lower() in [mlig_i.lower() for mlig_i in args.mlig]):
                    mligcatoms_ext[args.mlig.index(ligand.lower())] = int(
                        args.mligcatoms[args.mlig.index(ligand.lower())]) + initatoms
                if args.debug:
                    print((ligand.lower()))
                    print((args.mlig))
        ...
        if conats > 3:
            ...
            for comb in combs:
                ...
                if checkplanar(r[0], r[1], r[2], r[3]):
                    ...
                    ueq = vecdiff(r[random.randint(0, 3)],
                                  core3D.getAtomCoords(0))
                    break
        for i in range(0, Nogeom+1):
            ...
            while True:
                ...
                if planar and pos:
                    ...
                    args.maxd = R
                    if 'ax' in args.place:
                        newmol = setPdistanceu(
                            an3D, refbP, core3D.getAtomCoords(0), R, uax)
                    elif 'eq' in args.place:
                        P = getPointu(core3D.getAtomCoords(0), 100, ueq)
                        mindist = core3D.getfarAtomdir(P)
                        newmol = setPdistanceu(
                            an3D, refbP, core3D.getAtomCoords(0), R+mindist, ueq)
                else:
                    # get maximum distance in the correct direction
                    Pp0 = PointTranslatetoPSph(core3D.centermass(), [
                                               0.5, 0.5, 0.5], [0.01, theta, phi])
                    cmcore = core3D.centermass()
                    # get far away point in space
                    uP = getPointu(cmcore, 100, vecdiff(Pp0, cmcore))
                    mindist = core3D.getfarAtomdir(uP)
                    # Angstrom, distance of non-interaction
                    maxdist = mindist+float(args.maxd)
                    # Angstrom, distance of non-interaction
                    mindist = mindist+float(args.mind)
                    # get random distance, separated for i=0
                    ...
                    if ('theta1' in locals()):
                        ...
                        newmol2 = rotateRef(
                            tr3D2, refbP, [thetax, thetay, thetaz])
